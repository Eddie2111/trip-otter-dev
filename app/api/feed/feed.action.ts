...
function testLogger(scope: string, message: string, payloads: any) {
  console.log(`at scope: ${scope}, logging for: ${message}, payloads recieved: ${JSON.stringify(payloads)}`);
}
...
export async function getPublicFeed_v2(
  ...
  profileId?: string,
    ...
    // not active and requires unit testing
  testLogger("getPublicFeed_v2", "start", { skip, limit, profileId });
  const dummy = await runDBOperation(
    async () => {
      ...
      // const joinedtribeIds = profile.tribesJoined.map((id) => id.toString());
      console.log(profile);
      // const createdtribeIds = profile.tribesCreated.map((id) => id.toString());
      // testLogger("getPublicFeed_v2", "##########", { followingIds, joinedtribeIds, createdtribeIds });
  ...
  testLogger("getPublicFeed_v2", "end", dummy);
  return dummy;
}