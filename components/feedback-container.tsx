    ...
    if (formData.type === "review") {
      ...
      if (formData.bugMedia) {
        ...
        try {
          ...
          setIsUploadingMedia(false); // Ensure this is reset
          return; // Stop submission if media upload fails
        } finally {
  ...
  return (
    <DropdownMenu onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        <button
          className="fixed bottom-4 right-4 p-4 rounded-full bg-blue-600 text-white shadow-lg hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300 transition-all duration-300 ease-in-out z-50 transform hover:scale-105"
      ...
      <DropdownMenuContent
        side="top"
        align="end"
        className="w-80 md:w-96 p-0 shadow-xl rounded-lg overflow-hidden bg-white z-50"
      ...
      >
        {showThankYou ? (
          ...
          <>
            <DropdownMenuLabel className="flex justify-between items-center p-4 bg-gray-50 border-b border-gray-100">
            ...
            {errorMessage && (
              <div className="p-4 text-sm text-red-700 bg-red-100 border border-red-200 rounded-md mx-4 my-2">
                {errorMessage}
              </div>
            )}
            ...
            <DropdownMenuGroup className="p-4">
              {currentStep === 1 && (
                <div className="flex flex-col space-y-3">
                  <DropdownMenuItem
                    className="p-3 cursor-pointer flex items-center justify-between hover:bg-blue-50 rounded-md transition-colors"
                    onClick={() => handleTypeSelect("review")}
                    onSelect={(e) => e.preventDefault()}
                  >
                    <div className="flex items-center">
                      <MessageSquareText className="h-5 w-5 mr-3 text-blue-600" />
                      <span className="text-base text-gray-800">Review</span>
                    </div>
                    <ChevronRight className="h-4 w-4 text-gray-500" />
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    className="p-3 cursor-pointer flex items-center justify-between hover:bg-red-50 rounded-md transition-colors"
              ...
              {currentStep === 2 && formData.type === "review" && (
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    >
                      <FileText className="inline-block h-4 w-4 mr-1 text-gray-500" />
                      Scope
                    </label>
                    <select
                      id="reviewScope"
                  ...
                  <div>
                    <label
                      ...
                      className="block text-sm font-medium text-gray-700 mb-1"
                    >
                      <Pencil className="inline-block h-4 w-4 mr-1 text-gray-500" />
                      Your Review
                    </label>
                    <textarea
                      id="reviewContent"
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    <label
                      ...
                      className="block text-sm font-medium text-gray-700 mb-1"
                    >
                      <Paperclip className="inline-block h-4 w-4 mr-1 text-gray-500" />
                      Media (Optional)
                    </label>
                    <input
                      type="file"
